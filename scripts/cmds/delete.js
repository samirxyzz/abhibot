...
module.exports = {
  config: {
    ...
    guide: {
      en: "{pn} (Clean cache and temp files)\ {pn} <file.js> (Deletes specific command)\ {pn} images (Deletes downloaded images)"
    },
  },
  ...
  onStart: async function ({ args, api, event }) {
    ...
    try {
      if (fileName === "images") {
        ...
        api.sendMessage(`✅|Deleted all caches and temp files \from the system 💻 Directories|🗂.`, event.threadID);
      }
    } catch (err) {
      console.error(err);
