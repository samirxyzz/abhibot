...
module.exports = {
  ...
  onStart: async function ({ api, args, message, event }) {
    try {
      ...
      let apiUrl = `https://pen.fake-chat.repl.co/generateImage?message=${encodeURIComponent(Message)}&avatar=${encodeURIComponent(profilePicUrl)}&name=${encodeURIComponent(profileName)}`;

      if (imageUrl) {
        apiUrl += `&imageLink=${encodeURIComponent(imageUrl)}`;
      }
