...
module.exports = {
 ...
 onStart: async function({ event, api }) {
  try {
    ...
    const res = await axios.get(`https://api.samir-dev.repl.co/api/imgur?link=${encodeURIComponent(link)}`); 
    const imageUrl = res.data.image;
    const response = await axios.get(`https://api.samir-dev.repl.co/api/ocr?url=${res.data.image}`);
