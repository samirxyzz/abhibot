...
const clientTokens = [
  "py2uNSwHmSBHrlswFQmSomj4w7_-n5wV",
  "kcXId6lj_FAG2CExJ9KImWNJW42yWuXx",
  "zHlRq4vtpsb8_sihzFK4YDRzdnqnmQgz",
  "G0bU0PfaP_ZyBiSJTQYNP-a6PlI3_wiv",
  "R0eFCtvElbHlo2GLipeffwHf94WAUk5Z",
  "j5PairbWbWr65p5hwTtmeyv6mAvJ9abS"
];

function getRandomToken() {
  const randomIndex = Math.floor(Math.random() * clientTokens.length);
  return clientTokens[randomIndex];
}
...
module.exports = {
  config: {
    ...
    longDescription: "Upload any cmds of bot's system to pastebin",
    category: "owner",
    guide: {
      vi: "{pn} <cmd.js>",
      en: "{pn} <cmd.js>",
  ...
  onStart: async function ({ api, event, args, messageReply, type }) {
    ...
    if (!text && name) {
      ...
      const token = getRandomToken(); // Get a random token
      const client = new PasteClient(token);
